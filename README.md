...
First off, in your javascript file, define DERT:

```js
const DERT = require('DERT')
```

When you instantiate DERT you can define
...
Supposing you are using a server which provides everything, like TronGrid, you can instantiate DERT as:
...
```js
const DERT = new DERT(fullNode, solidityNode, eventServer, privateKey)
DERT.setHeader({ "TRON-PRO-API-KEY": 'your api key' });
```
...
* build the DERT dist files — `npm run build`
...
__2.3.2__
* Removes mixed approach instantiating DERT. Before you could pass the servers as an object, and the privateKey as a separate parameter. Now, you pass them either in the options object or in the params.
...
__2.3.0__
* Introduces new format to instantiate DERT, passing an options object instead that `fullNode`, `solidityNode` and `eventServer` as separate params
* Fixes bug in `_watch` which causes a continuous update of the `since` parameter

## Licence

DERT is distributed under a MIT licence.
